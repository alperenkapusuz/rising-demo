...
const DashboardCard = () => {
  ...
  const totalDataUsage =
    parseInt(undefinedControl(query.data?.totalDataUsage)) > 1000
      ? (parseInt(undefinedControl(query.data?.totalDataUsage)) / 1000).toFixed(3)
      : parseInt(undefinedControl(query.data?.totalDataUsage));
  const dailyUsage =
    parseInt(undefinedControl(query.data?.dailyUsage)) > 1000
      ? (parseInt(undefinedControl(query.data?.dailyUsage)) / 1000).toFixed(3)
      : parseInt(undefinedControl(query.data?.dailyUsage));
  ...
  const cardData: Array<ICard> = [
    ...
    },
    {
      ...
      value: (
        <>
          {lastChargeAmount} <span style={{ fontSize: '15px' }}>{lastCharge}</span>
        </>
      ),
      bgColor: 'gray',
      fontWeight: 'medium',
